...
uv run comiket-plan build-next C109 --follow follow-a.json --follow follow-b.json --previous previous.json --profile-pattern C108 --include-previous-only --output data/derived/C109.json --report data/derived/C109-report.json
...
Use `build-next` for the current MVP input flow. Repeat `--follow` for exports from separate X accounts. Merge them by case-insensitive X handle, preserve every source filename, and never merge by display name. `--profile-pattern` filters the display name plus bio; for a virtual C109 rehearsal, use `C108`. `--include-previous-only` forms a union with the previous route list. Carry forward only `priority` and `genre_short`; reset placement, visit state, and purchase state. A follow-profile display name is a creator-name candidate, not a confirmed circle name.
